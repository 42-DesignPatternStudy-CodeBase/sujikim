package iteratorPattern.example.menu;

import java.util.HashMap;
import java.util.Map;

public class CafeMenuVendor implements MenuVendor {

    private Map<String, MenuItem> menuItems;
    
    public CafeMenuVendor() {
        menuItems = new HashMap<String, MenuItem>();

        addMenuItem("베지 버거와 에어 프라이", "통밀빵, 상추, 토마토, 감자 튀김이 첨가된 베지 버거", true, 3.99);
        addMenuItem("오늘의 스프", "샐러드가 곁들여진 오늘의 스프", false, 3.69);
        addMenuItem("부리토", "통 핀토콩과 살사, 구아카몰이 곁들여진 푸짐한 부리토", true, 4.29);
    }

    public void addMenuItem(String name, String description, boolean vegetarian, double price) {
        MenuItem newItem = new MenuItem(name, description, vegetarian, price);
        menuItems.put(name, newItem);
    }

    @Override
    public ClassicIterator createIterator() {
        return new CafeMenuIterator(menuItems);
    }
    
    
}
